...
| **ORM / DB** | SQLAlchemy + Postgres (Neon free tier) in production, SQLite for tests | Postgres over SQLite in production because the app is genuinely multi-user/concurrent (many chat sessions writing `Message`/`UsageLog` rows at once) — SQLite's single-writer lock isn't a good fit for that. Neon specifically because it's a real managed Postgres with a usable free tier and no server to provision. SQLite for tests because the suite needs to spin up and tear down a fresh schema per test in-process with zero external dependencies — swapping the engine URL is the only difference (`tests/conftest.py`). |
| **Frontend** | Jinja2-rendered pages + vanilla JS, no framework, no build step | The assignment left this open, and the actual UI surface (catalog, agent detail, auth, chat, admin) is four page types with straightforward client-server data flow — not enough state or component reuse to justify React/Vue's overhead, and a build step is one more thing that can break "loads and functions without you present to explain it." Shared behavior (`esc()`/`colorFor()`/`toast()`/`apiErrorMessage()`) lives in `base.html` and is reused across every page without a bundler. |
| **LLM provider** | Google Gemini (`google-genai`, `gemini-flash-latest`) | Assignment allowed any provider. Picked Gemini for a genuinely usable free tier while iterating across ~100 agents, and because the call is isolated behind one function (`get_reply()` in `app/llm.py`) — swapping to OpenAI/Anthropic means rewriting that one function, not touching routing, auth, or the pipeline. |
| **Rate limiting** | `slowapi`, keyed by authenticated user (IP fallback only for unauthenticated requests) | A small, FastAPI-native library with a decorator API (`@limiter.limit("15/minute")`) rather than middleware threaded through every route. Keyed by user rather than IP specifically because IP-based limiting would let one shared-network user's chat activity throttle everyone else behind the same IP/NAT — `get_current_user` sets `request.state.user` so the limiter's key function can use it. |
| **Deployment** | Render (`render.yaml`), free tier | Chosen over Railway/Fly.io for the least ceremony for a FastAPI app specifically: `render.yaml` declares the build/start commands directly, HTTPS is automatic with no separate certificate step, and the free web-service tier doesn't require a credit card. The tradeoff is cold starts after ~15 minutes idle (see Known Limitations). |
...
Tests run against an in-memory SQLite DB via dependency override, no external services required. CI (`.github/workflows/test.yml`) runs this suite on every push/PR to `main`.

## AI-assisted development

**Claude Code** (Anthropic) was used throughout this project's development — architecture decisions, implementation (both backend and frontend), and this README were all done in collaboration with it.
...
- **The admin credential is a plaintext environment variable, not a hashed secret.** `verify_admin` (`app/admin_auth.py`) compares the submitted HTTP Basic password directly against `ADMIN_USER`/`ADMIN_PASSWORD` via `secrets.compare_digest` — unlike regular user passwords, which are bcrypt-hashed and never stored in plaintext anywhere, the admin password exists in plaintext in the environment for as long as the process runs.
...
- **Frontend behavior (the redesign, toast notifications, error formatting) is verified manually in a real browser, not covered by `pytest`** — everything server-side it depends on (validation, ordering, auth) is.
- **HTTPS is enforced by Render, not application code.** Render redirects `http://` → `https://` at its edge before the request reaches the app, so no `HTTPSRedirectMiddleware` was added — `render.yaml` doesn't currently pass `--proxy-headers` to uvicorn, and enabling that middleware without it would cause a redirect loop rather than add any real protection.
